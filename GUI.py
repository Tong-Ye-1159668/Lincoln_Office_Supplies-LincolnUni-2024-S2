...
class LOSApp:
    def __init__(self, root, controller):
        ...
        # Reset button
        reset_button = tk.Button(sidebar, text="Reset Input", bg='#ef233c', fg='white', font=("Arial", 14, "bold"),
    ...
    def setup_frame1(self):
        ...
        self.customer_info_text = tk.Text(customer_info_frame, height=5, width=40, bg="#caf0f8")
        ...
        self.order_details_text = tk.Text(order_details_frame, height=8, width=80, bg="#caf0f8")
        self.order_details_text.grid(row=0, column=0)

        submit_order_button = tk.Button(order_details_frame, text="Submit Order", command=self.submitOrder)
    ...
    def setup_frame2(self):
        ...
        display_orders_button = tk.Button(customer_info_frame, text="Display Orders", command=self.displayCustomerOrders)
        display_orders_button.grid(row=0, column=2, padx=10)

        self.customer_orders_text = tk.Text(self.frame2, height=20, width=80, bg="#caf0f8")
    ...
    def displayCustomerOrders(self):
        customer_name = self.customer_combobox2.get()
        customer = self.Controller.findCustomerByName(customer_name)
        if customer:
            self.customer_orders_text.delete(1.0, tk.END)
            orders = self.Controller.displayCustomerOrders(customer_name)
            if orders:
                for order in orders:
                    self.customer_orders_text.insert(tk.END, f"Order Info: {order}\n")
            else:
    ...
    def setup_frame3(self):
        ...
        display_payments_button = tk.Button(customer_info_frame, text="Display Payments", command=self.displayCustomerPayments)
        display_payments_button.grid(row=0, column=2, padx=10)

        self.customer_payments_text = tk.Text(self.frame3, height=20, width=80, bg="#caf0f8")
    ...
    def displayCustomerPayments(self):
        customer_name = self.customer_combobox3.get()
        customer = self.Controller.findCustomerByName(customer_name)
        if customer:
            self.customer_payments_text.delete(1.0, tk.END)
            payments = self.Controller.displayCustomerPayments(customer_name)
            if payments:
                for payment in payments:
                    self.customer_payments_text.insert(tk.END, f"Payment info: {payment}\n")
            else:
    ...
    def setup_frame4(self):
        ...
        self.all_customers_text = tk.Text(customer_info_frame, height=50, width=120, bg="#e0ffff")
    ...
    def display_all_customers(self):
        ...
        for customer in customers:
            self.all_customers_text.insert(tk.END, f"Customer ID: {customer.customerID}, Name: {customer.customerName}\n")
    ...
    def setup_frame5(self):
        ...
        self.all_orders_text = tk.Text(orders_info_frame, height=50, width=120, bg="#caf0f8")
    ...
    def displayAllOrders(self):
        ...
        for order in orders:
            self.all_orders_text.insert(tk.END, f"Order Info: {order}\n")
    ...
    def setup_frame6(self):
        ...
        self.all_payments_text = tk.Text(payments_info_frame, height=50, width=120, bg="#caf0f8")
    ...
    def display_all_payments(self):
        ...
        for payment in payments:
            self.all_payments_text.insert(tk.END, f"{payment}\n")
    ...
    # Method to reset the input fields in the "Add New Order" page
    def reset_frame1(self):
        '''Reset the input fields in the "Add New Order" page'''
        self.show_frame(self.frame1)
    ...
    # Method to create a new order for a selected customer
    def create_new_order(self):
        '''Create a new order for a selected customer'''
        customer_name = self.customer_combobox.get()
        customer = self.Controller.findCustomerByName(customer_name)
        if customer:
            self.current_order = self.Controller.createOrder(customer_name)
            ...
            self.order_details_text.delete(1.0, tk.END)  # Clear the order details text box
        else:
            messagebox.showerror("Error", "Customer not found.")
    ...
    def add_product(self):
        ...
        product = self.Controller.findProductByName(product_name)
        if product:
            self.Controller.addOrderItem(self.current_order, product, quantity)
            self.order_details_text.insert(tk.END, f"{product_name} - ${product.productPrice} * {quantity} =  ${quantity*product.productPrice:.2f} added to order.\n")
        else:
            messagebox.showerror("Error", "Product not found.")

    # Method to submit the current order
    def submitOrder(self):
        '''Submit the current order'''
        if self.current_order:
            totalPrice = self.Controller.submitOrder(self.current_order)
            self.order_details_text.insert(tk.END, f"Price before GST: ${totalPrice*0.85:.2f}\n")
            self.order_details_text.insert(tk.END, f"GST: ${totalPrice*0.15:.2f}\n")
            self.order_details_text.insert(tk.END, f"totalPrice: ${totalPrice:.2f}\n")
            self.order_details_text.insert(tk.END, f"Your order has been submitted successfully.\n")
            self.customer_info_text.delete(1.0, tk.END)
            self.customer_info_text.insert(tk.END, f"Customer ID: {self.current_customer.customerID}\n")
            self.customer_info_text.insert(tk.END, f"Name: {self.current_customer.customerName}\n")
            self.customer_info_text.insert(tk.END, f"Balance: {self.current_customer.customerBalance:.2f}\n")
        else:
            messagebox.showerror("Error", "No active order to submit.")

    # Method to process a payment
    def process_payment(self):
        '''Process a payment for a selected customer'''
        customer_name = self.customer_combobox.get()
        try:
            ...
            return
        if amount < self.current_order.calcTotal():
            messagebox.showerror("Error", "Payment amount is less than the order total.")
            return

        customer = self.Controller.findCustomerByName(customer_name)
        if customer:
            self.Controller.makePayment(customer_name, amount)
